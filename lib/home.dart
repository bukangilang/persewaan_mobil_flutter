...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _getDataFromStrapi() async {
    ...
    var response = await http
        .get(Uri.parse('http://localhost:1337/api/penyewas'), headers: headers);

    dataJson = jsonDecode(response.body);

    setState(() {
      totalData = dataJson["meta"]["pagination"]["total"];
      //print(dataJson["data"][1]["attributes"]["nama"]);
      _counter++;
    });
  }
    ...
    return Scaffold(
      ...
      body: ListView.builder(
          ...
          itemBuilder: (data, index) {
            return ListTile(
              onTap: () {
                var nama = dataJson["data"][index]["attributes"]["nama"];
                var nik = dataJson["data"][index]["attributes"]["nik"];
              ...
              title: Text(dataJson["data"][index]["attributes"]["nama"]),
            );
          }),
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => const CreatePenyewa()),
          );
        },
        tooltip: 'Increment',
